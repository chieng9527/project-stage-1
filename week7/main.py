from fastapi import FastAPI, Request, Form, Path, status, Depends, Body
...
app.add_middleware(
    ...
    https_only=True,  # 確保只在 HTTPS 下傳輸
    same_site="lax"    # 防止 CSRF 攻擊
)
# 設定模板引擎與靜態文件
...
def is_valid_username(username: str) -> bool:
    """檢查 username 是否符合格式（只允許 1~50 個英數字）"""
    return bool(re.fullmatch(r"^[a-zA-Z0-9]{1,50}$", username))

def fetch_member_by_username(username: str):
    """根據 username 查詢會員資訊"""
...
def update_member_name_in_db(user_id: int, new_name: str):
    """更新會員姓名"""
    with get_db_connection() as conn:
        cursor = conn.cursor()
...
# === 會員查詢 API ===
@app.get("/api/member")
async def query_member(request: Request, username: str | None = None):
    session_user = get_session_user(request)
    if not session_user["SIGNED_IN"] or not username or not is_valid_username(username):
        return JSONResponse({"data": None})

    member_data = fetch_member_by_username(username)
    return JSONResponse({"data": member_data if member_data else None})


# === 更新會員姓名 API ===
@app.patch("/api/member")
async def update_member_name(request: Request, payload: dict[str, str] = Body(...)):
    session_user = get_session_user(request)
    if not session_user["SIGNED_IN"]:
        return JSONResponse({"error": True})

    new_name = payload.get("name", "").strip()
    if not new_name or len(new_name) > 50 or not re.fullmatch(r"^[a-zA-Z0-9\u4e00-\u9fa5 ]+$", new_name):
        return JSONResponse({"error": True})

    try:
        update_member_name_in_db(session_user["USER_ID"], new_name)
        request.session["USER_NAME"] = new_name
        return JSONResponse({"ok": True})
    except mysql.connector.Error:
        return JSONResponse({"error": True})  # 避免洩露內部錯誤細節
